CREATE DEFINER=`root`@`localhost` PROCEDURE `PayCompleteMetrics`()
BEGIN
...
SELECT 
    ...
    convert(trim(BOTH '"' from JSON_EXTRACT(JSON_EXTRACT(payment_Value, '$.values'),
            '$.approvalamount')),unsigned integer) AS 'ApprovalAmount',
...
select count(InvoiceNo) 'Total Invoices', sum(ApprovalAmount) 'Total Amount' from flowable.finalPayReport;
