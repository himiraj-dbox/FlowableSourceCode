    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if (!skipAuthenticationCheck(request)) {
            ...
            if (token != null) {
            	...
            	if(null == request.getRequestURI() || request.getRequestURI().length() <40 || !request.getRequestURI().substring(0, 36).equalsIgnoreCase("/DB-task/app/rest/process-instances/")) {
                    redirectOrSendNotPermitted(request, response, null);
                    return; // no need to execute any other filters
            	}
               // redirectOrSendNotPermitted(request, response, null);
            	RemoteToken token2 = new RemoteToken();
            	token2.setId("BiqNCibor29Db1AS3HwFRw==");
            	token2.setUserId("admin");
            	token2.setValue("pbxyGuwXqhxZvPexmphNog==");

                try {
                    FlowableAppUser appUser = userCache.get(token2.getUserId());
                    
                    SecurityContextHolder.getContext().setAuthentication(new RememberMeAuthenticationToken(token2.getId(),
                        appUser, appUser.getAuthorities()));
