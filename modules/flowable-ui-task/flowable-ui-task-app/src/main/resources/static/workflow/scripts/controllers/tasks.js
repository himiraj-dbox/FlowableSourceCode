  ...
  .controller('TasksController', ['$rootScope', '$scope', '$translate', '$http', '$timeout','$location', '$modal', '$popover', 'appResourceRoot', 'CommentService', 'TaskService', '$routeParams', 'AppDefinitionService',
        function ($rootScope, $scope, $translate, $http, $timeout, $location, $modal, $popover, appResourceRoot, CommentService, TaskService, $routeParams, AppDefinitionService) {
            ...
            $scope.model.stateFilterOptions = [
              ...
              $scope.model.taxInboxOption = [
                { id: 'open', title: 'OPEN',iconClass:'fa fa-inbox' },
                { id: 'completed', title: 'COMPLETED',iconClass:'' }
            ...
            if ($scope.account && $scope.account.groups && $scope.account.groups.length > 0) {
              ...
              $scope.model.viewMainInbox = true;
               $scope.model.viewSubInbox = true;

               $scope.togglemainInbox = function(forcePropagateToRootScope) {
$scope.model.viewMainInbox = !$scope.model.viewMainInbox;
               }

               $scope.toggleSubInbox = function(forcePropagateToRootScope) {
$scope.model.viewSubInbox = !$scope.model.viewSubInbox;
               }

                $scope.showmainInbox = function() {
$scope.model.viewMainInbox = true;
               }

               $scope.showSubInbox = function() {
$scope.model.viewSubInbox = true;
               }

                 $scope.hidemainInbox = function() {
$scope.model.viewMainInbox = false;
               }

               $scope.hideSubInbox = function() {
$scope.model.viewSubInbox = false;
               }
            ...
            $scope.selectStateFilter = function (state) {
              ...
              $scope.selectStateFilterTask = function (state) {
                if(state !== $scope.model.filter.param.state) {
                    if(state.id == 'open' || state.id == 'completed'){
                    $scope.model.filter.param.state = state;
                    $scope.model.filter.param.assignment =  'involved';
                    } else {
                       $scope.model.filter.param.assignment =   state.id;
                    }
                     $scope.resetPaging();
                    $scope.collapseFilter();
            ...
            $scope.refreshFilter = function () {
                ...
                TaskService.queryTasks(data).then(function (response) {
                    ...
                    if (response.start === 0) {
                        ...
                        $rootScope.root.processInstanceIDPDF = $scope.model.tasks[0].processInstanceId ;
                        $scope.selectedQueueToDisplay =  $scope.selectedTask.name;
                    ...
                    if ($rootScope.root.selectedTaskId) {
                        for (var i = 0; i < response.data.length; i++) {
                            if (response.data[i].id == $rootScope.root.selectedTaskId) {
                                ...
                                $rootScope.root.processInstanceIDPDF = $scope.model.tasks[i].processInstanceId ;
                                break;
                            }
                        }
                    ...
                    var taskNameIndex = 0 ;
                    $scope.model.uniqueTaskName = [];
                    for (var taskNameIndex=0; taskNameIndex < $scope.model.tasks.length; taskNameIndex++) {
                       if(!$scope.model.uniqueTaskName.contains($scope.model.tasks[taskNameIndex].name)){
                           $scope.model.uniqueTaskName.push($scope.model.tasks[taskNameIndex].name);
                       }
                    }

                });
            };
            $scope.queueToDisplay = function(obj) {
                $scope.selectedQueueToDisplay = obj;
              
          } 
          $scope.selectedQueueToDisplay = '';
